  ...
  setSlideVisibility(event) {
    this.sliderItemsToShow.forEach((item, index) => {
      ...
      if (index === this.currentPage - 1) {
        if (linkElements.length) linkElements.forEach(button => button.removeAttribute('tabindex'));
        item.setAttribute('aria-hidden', 'false');
        item.removeAttribute('tabindex');
      } else {
        if (linkElements.length) linkElements.forEach(button => button.setAttribute('tabindex', '-1'));
...
class VariantSelects extends HTMLElement {
  ...
  updateVariantStatuses() {
    const addButtonText = document.querySelector(`#ProductSubmitButton-${this.dataset.section} span`);
  ...
  renderProductInfo() {
    ...
    fetch(`${this.dataset.url}?variant=${requestedVariantId}&section_id=${sectionId}`)
      ...
      .then((responseText) => {
        ...
        const volumeNoteSource = html.getElementById(`Volume-Note-${sectionId}`);
        ...
        if (volumeNote) volumeNote.classList.toggle('visibility-hidden', volumeNoteSource.classList.contains('visibility-hidden'));
